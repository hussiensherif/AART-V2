...
DEFAULT_PARAMS = {
    ...
    'priorities': {},  # {hour: weight} where weight in [-10, +10]; default empty = all neutral
}
...
def _initial_assign_fixed_start(demand, n, params, allowed_off_days):
    ...
    for st_c in valid_starts:
        br_c = _valid_breaks(st_c, params)[0]
        total_c = 0
        for di_c in range(7):
            for td_c, hr_c in _covers(st_c, br_c, di_c, params):
    ...
    for i in range(n):
        best_start, best_break, best_off, best_score = None, None, None, -1e9
...
# =============================================================================
# PRIORITY-BASED DEMAND ADJUSTMENT
# =============================================================================
def _apply_priorities(demand, priorities):
    """
    Apply priority weights to demand matrix, producing effective_demand.

    Args:
        demand: dict {day_name: [24 ints]} — original demand matrix
        priorities: dict {hour_int: weight} — weights from -10 to +10

    Returns:
        effective_demand: dict {day_name: [24 floats]} — adjusted demand

    Formula: effective_demand[day][h] = demand[day][h] * (1 + clamp(priority[h], -10, 10) / 10)
    """
    effective = {}
    for day in DAYS:
        effective[day] = []
        for h in range(24):
            weight = priorities.get(h, 0)
            weight = max(-10, min(10, weight))  # clamp
            multiplier = 1 + weight / 10
            effective[day].append(demand[day][h] * multiplier)
    return effective
        ...
        for i in order:
            ...
            for st in valid:
                ...
                for br in valid_breaks:
                    ...
                    # This prevents painting into a corner (e.g., overnight Fri
                    # shift that blocks all Saturday starts with tight rest).
                    flex_penalty = 0.0
                    s_end = _shift_end(st, params, di)
                    s_on = _is_overnight(st, params, di)
                    
                    if unprocessed_next and day_off[i] != next_di:
                        # Count how many starts would be valid on next_di given this shift
                        next_valid_count = 0
                        for ns in range(24):
                            if not params.get('night_shift_enabled', True):
                            ...
                            if _calc_rest(s_end, di, s_on, next_di, ns) >= params['min_rest']:
                                next_valid_count += 1
                        # Moderate penalty — avoid blocking next day but don't dominate scoring
...
def assign_shifts(da_list_df, demand_df, carryover_df=None, params=None):
    ...
    print(f"Assigning shifts (v12.5 Tunable, {mode}, fixed_start={is_fixed}, "
    ...
    for store in stores:
        ...
        # Apply priorities to get effective demand for optimization
        priorities = params.get('priorities', {})
        effective_demand = _apply_priorities(demand, priorities)
        
        print(f"   Store {store}: {n} DAs")
        
        best_gap, best_shifts, best_day_off, zeros = _run_store_optimization(effective_demand, n, params)
        
        # Report gap against ORIGINAL demand (not effective)
        coverage = _calc_coverage(best_shifts, n, params)
        actual_gap = sum(max(0, demand[DAYS[di]][h] - coverage[di][h]) for di in range(7) for h in range(24))
        actual_zeros = sum(1 for di in range(7) for h in range(24) if demand[DAYS[di]][h] > 0 and coverage[di][h] == 0)
        print(f"      Gap (effective): {best_gap}, Gap (actual): {actual_gap}, Zero-coverage: {actual_zeros}")
...
def run_roster(input_file, output_file=None, store=None, custom_params=None):
    print("=" * 60)
    print("DA ROSTERING ENGINE v12.5 Tunable")
    print("=" * 60)
